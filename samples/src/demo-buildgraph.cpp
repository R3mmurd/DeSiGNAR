...
int main()
{
    ...
    GT fg = full_graph<GT>(5, [&i](GT::Node* p) { p->get_info() = ++i; });

    Dot().write_graph(fg, "full-graph.dot");

    i = 0;

    DGT fdg = full_graph<DGT>(3, [&i](DGT::Node* p) { p->get_info() = ++i; });

    DDot().write_graph(fdg, "full-digraph.dot");

    i = 0;

    GT rg = ring_graph<GT>(10, 4, [&i](GT::Node* p) { p->get_info() = ++i; });
    ...
    DGT rdg =
        ring_graph<DGT>(10, 4, [&i](DGT::Node* p) { p->get_info() = ++i; });
    ...
    GT rgna =
        random_graph<GT>(5, 7, [&i](GT::Node* p) { p->get_info() = ++i; });
    ...
    DGT rdgna =
        random_graph<DGT>(5, 7, [&i](DGT::Node* p) { p->get_info() = ++i; });

    DDot().write_graph(rdgna, "random-digraph-na.dot");

    i = 0;

    GT rghpnc = er_random_graph<GT>(5, 0.8, false,
                                    [&i](GT::Node* p) { p->get_info() = ++i; });

    Dot().write_graph(rghpnc, "random-graph-hp-nc.dot");

    i = 0;

    GT rghpc = er_random_graph<GT>(5, 0.8, true,
                                   [&i](GT::Node* p) { p->get_info() = ++i; });
    ...
    GT rglpnc = er_random_graph<GT>(10, 0.2, false,
                                    [&i](GT::Node* p) { p->get_info() = ++i; });
    ...
    GT rglpc = er_random_graph<GT>(10, 0.2, true,
                                   [&i](GT::Node* p) { p->get_info() = ++i; });
    ...
    GT hexagonal_grid = build_grid<GT>(4, 4, GridType::HEXAGONAL,
                                       [&i](GT::Node* p, nat_t, nat_t)
                                       { p->get_info() = ++i; });
    ...
    GT octagonal_grid = build_grid<GT>(4, 4, GridType::OCTAGONAL,
                                       [&i](GT::Node* p, nat_t, nat_t)
                                       { p->get_info() = ++i; });
