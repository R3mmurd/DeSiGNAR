...
| Module | Covers |
| :----- | :----- |
| Data structures | B-trees, B+ trees, skip lists, tries, LRU caches, bloom filters, union-find, the double-ended queue (`Deque`), and the mergeable-heap family (pairing, leftist, binomial, Fibonacci), plus the foundational array/list/stack/queue/heap/map/set family at the top level of `include/`. |
...
| Geometry | 2D points/vectors/segments/polygons/triangles, closest-pair, quadtrees, Voronoi diagrams, Delaunay triangulation, polygon triangulation (ear-clipping and monotone-decomposition, side by side) with the art-gallery guard bound, polygon partition (monotone, trapezoidal, convex), line arrangements, and convex/non-convex polygon intersection; plus a minimal 3D point/plane pair for point-in-polyhedron testing. |
| Sorting | Comparison sorts, the non-comparison counting/radix/bucket sorts, and the heterogeneous-key search helpers backing the map/set containers. |
| Algorithms | Generic algorithms over iterators/containers: string algorithms, the container/iterator algorithm mixins reused everywhere else, elementary number theory (gcd, modular exponentiation/inverse, Chinese remainder theorem), the classic randomized algorithms (randomized order statistics, Miller-Rabin primality testing, reservoir sampling, Karger's min-cut), textbook RSA public-key cryptography, a generic backtracking search (N-Queens), the classic dynamic-programming examples (matrix-chain order, longest common subsequence, optimal polygon triangulation, 0/1 knapsack), and — for two classic NP-hard problems — an exact/DP technique next to a heuristic: fractional knapsack's optimal greedy solution, and TSP's nearest-neighbor heuristic next to its MST-based 2-approximation, plus greedy graph coloring (Welsh-Powell). |
| Hashing | `SeparateChainingHashTable`/`OpenAddressingHashTable` underlying `HashMap`/`HashSet`, `ExtendibleHashTable` (directory-of-buckets, splitting/doubling on overflow), a hash-function collection (including MD5/SHA-1/SHA-256 for security use), and `HashChain` for iterated hash-chain (S/KEY-style OTP) verification. |
| Concurrency | Thread pool, concurrent map, graph agent + concurrent graph for parallel graph exploration. |
| Compilers | Grammars with FIRST/FOLLOW computation and optional operator precedence/associativity declarations, a lexer, an LL(1) predictive parser, and the full LR family (SLR(1), canonical LR(1), LALR(1)) — every parser drives yacc-style semantic actions live during the parse to build a real, typed AST, suitable for an interpreter or a translator. `Parser<NodeType>` is a Bison/Flex-style facade on top of all of it: owns its own grammar and lexer, takes one token/rule at a time with an action attached, and turns a source string straight into an AST via a runtime-selectable LL/SLR/LR/LALR backend. Beyond parsing: a three-address-code IR (`IRModule`/`IRFunction`/`IRBasicBlock`, built via `IRBuilder`), a constant-folding optimization pass, and target-parametrized stack-frame layout (`compute_frame_layout()`, against `TargetInfo` for x86-64/AArch64/RISC-V) — designed as a natural lowering target for a future consumer, none committed. |
...
| Linear algebra | Vector and matrix types with the usual arithmetic, Strassen's algorithm for matrix multiplication, and `Polynomial`/the Fast Fourier Transform (with FFT-based polynomial multiplication). |
