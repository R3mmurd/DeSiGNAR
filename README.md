...
| Algorithms | Generic algorithms over iterators/containers: string algorithms, the container/iterator algorithm mixins reused everywhere else, elementary number theory (gcd, modular exponentiation/inverse), the classic randomized algorithms (randomized order statistics, Miller-Rabin primality testing, reservoir sampling, Karger's min-cut), a generic backtracking search (N-Queens), the classic dynamic-programming examples (matrix-chain order, longest common subsequence, optimal polygon triangulation, 0/1 knapsack), and — for two classic NP-hard problems — an exact/DP technique next to a heuristic: fractional knapsack's optimal greedy solution, and TSP's nearest-neighbor heuristic next to its MST-based 2-approximation, plus greedy graph coloring (Welsh-Powell). |
