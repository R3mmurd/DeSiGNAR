...
namespace Designar
{
    ...
    public:
        Path() : ptr_owner_graph(nullptr), list()
        {
            // Empty
        }

        Path(const GT& graph) : ptr_owner_graph(const_cast<GT*>(&graph)), list()
        {
            // Empty
        }

        Path(GT* graph_ptr) : ptr_owner_graph(graph_ptr), list()
        {
            // Empty
        }
        ...
        Path(Path&& path) : ptr_owner_graph(nullptr), list()
        {
        ...
        void insert(NodeType* s)
        {
            ...
            if (ptr_arc == nullptr)
            {
                throw std::logic_error(
                    "There is not arc between last && current node");
            }
        ...
        {
            return list.template map<Node<GT>*, SLList<Node<GT>*>>(
                [](auto& pd) { return pd.node; });
        }

        SLList<Arc<GT>*> arcs() const
        {
            return list.template map_if<Arc<GT>*>(
                [](auto& pd) { return pd.arc; },
                [](auto& pd) { return pd.arc != nullptr; });
        }
