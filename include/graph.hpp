...
namespace Designar
{
    ...
    public:
        ...
        {
            return filter_it<ContainerRet>(const_me().nodes_begin(),
                                           const_me().nodes_end(), pred);
        }
        ...
        {
            return map_if_it<ContainerRet>(
                const_me().nodes_begin(), const_me().nodes_end(),
                std::forward<Op>(op), std::forward<Pred>(pred));
        }

        template <typename RetT, class Op>
        RetT fold_nodes(const RetT& init_val, Op& op) const
        {
            return fold_it<RetT>(const_me().nodes_begin(),
                                 const_me().nodes_end(), init_val, op);
        }
        ...
        {
            return filter_it<ContainerRet>(const_me().arcs_begin(),
                                           const_me().arcs_end(), pred);
        }
        ...
        {
            for (auto it = const_me().arcs_begin(); it != const_me().arcs_end();
                 ++it)
            {
                if (pred(*it))
                {
        ...
        void remove_arc_if(Pred&& pred)
        {
            remove_if_it(me().arcs_begin(), me().arcs_end(),
                         std::forward<Pred>(pred));
        }

        SLList<Arc*> arcs() const
        {
            return to_list_it<Arc*>(const_me().arcs_begin(),
                                    const_me().arcs_end());
        }
        ...
        {
            return filter_it<ContainerRet>(const_me().arcs_begin(p),
                                           const_me().arcs_end(p), pred);
        }
        ...
        ContainerRet map_adjacent_arcs_if(Node* p, Op& op, Pred& pred) const
        {
            return map_if_it<ContainerRet>(const_me().arcs_begin(p),
                                           const_me().arcs_end(p), op, pred);
        }

        template <typename RetT = ArcInfoType,
                  class ContainerRet = SLList<RetT>, class Op, class Pred>
        ContainerRet map_adjacent_arcs_if(Node* p, Op& op,
        ...
        template <typename RetT = ArcInfoType,
                  class ContainerRet = SLList<RetT>, class Op, class Pred>
        ContainerRet map_adjacent_arcs_if(Node* p, Op&& op = Op(),
                                          Pred&& pred = Pred()) const
        {
            return map_if_it<ContainerRet>(
        ...
        {
            return fold_it<RetT>(const_me().arcs_begin(p),
                                 const_me().arcs_end(p), init_val, op);
        }

        template <typename RetT, class Op>
        RetT fold_adjacent_arcs(Node* p, const RetT& init_val,
                                Op&& op = Op()) const
        {
            return fold_it<RetT>(const_me().arcs_begin(p),
                                 const_me().arcs_end(p), init_val,
                                 std::forward<Op>(op));
        }
        ...
        {
            return to_list_it<Arc*>(const_me().arcs_begin(p),
                                    const_me().arcs_end(p));
        }

        void reset_all_node_tag(GraphTag tag) const
        {
            for_each_node([&tag](Node* node) { node->unvisit(tag); });
        }

        void reset_all_node_tag() const
        {
            for_each_node([](Node* node) { node->reset_tag(); });
        }

        void reset_all_arc_tag(GraphTag tag) const
        {
            for_each_arc([&tag](Arc* arc) { arc->unvisit(tag); });
        }

        void reset_all_arc_tag() const
        {
            for_each_arc([](Arc* arc) { arc->reset_tag(); });
        }
        ...
        {
            for_each_node([](Node* node) { node->cookie() = nullptr; });
        }

        void reset_arc_cookies() const
        {
            for_each_arc([](Arc* arc) { arc->cookie() = nullptr; });
        }

        void reset_node_counter() const
        {
            for_each_node([](Node* node) { node->counter() = 0; });
        }

        void reset_arc_counter() const
        {
            for_each_arc([](Arc* arc) { arc->counter() = 0; });
        }
        ...
        {
            for_each_node([](Node* node) { node->reset(); });
        }

        void reset_arcs() const
        {
            for_each_arc([](Arc* arc) { arc->reset(); });
        }
    };

    template <typename GT, class Node, class Arc, typename NodeInfoType,
              typename ArcInfoType>
    template <class RetT, class It>
    RetT*
    BaseGraph<GT, Node, Arc, NodeInfoType, ArcInfoType>::nth_graph_element(
        const It& a, const It& b, nat_t pos)
    {
        for (It i = a; i != b; ++i)
        {
            if (pos == 0)
    ...
    template <typename GT, class Node, class Arc, typename NodeInfoType,
              typename ArcInfoType>
    void BaseGraph<GT, Node, Arc, NodeInfoType, ArcInfoType>::copy_graph(
        const GT& src, GT& tgt)
    {
    ...
    class Graph
        : public BaseGraph<Graph<NodeInfo, ArcInfo, GraphInfo>,
                           ...
                           GraphArc<GraphNode<NodeInfo, ArcInfo, GraphInfo>,
    ...
    public:
        Graph() : info(), num_nodes(0), node_list(), num_arcs(0), arc_list()
        {
            // empty
        }

        Graph(const GraphInfo& _info) : info(_info), num_nodes(0), num_arcs(0)
        {
            // empty
        }
        ...
        Graph(const Graph& g) : info(g.info), num_nodes(0), num_arcs(0)
        {
        ...
        Node* get_first_node() const
        {
            ...
            return &dl_to_node(const_cast<DL&>(node_list).get_next())
                        ->get_item();
        }
        ...
        {
            GNode* node =
                insert_gnode(new GNode(Node(std::forward<NodeInfo>(info))));
            return &node->get_item();
        }
        ...
        public:
            NodeIterator() : Base(), graph_ptr(nullptr)
            {
                // empty
            }
            ...
            NodeIterator(NodeIterator&& it) : NodeIterator()
            {
                swap(it);
            }
        ...
        public:
            ArcIterator() : Base(), graph_ptr(nullptr)
            {
                // empty
            }
            ...
            ArcIterator(ArcIterator&& it) : ArcIterator()
            {
                swap(it);
            }
        ...
        public:
            ...
            Arc* get_current()
            {
                return &dl_to_adjacent_arc(Base::get_current())
                            ->get_item()
                            ->get_item();
            }
    ...
    template <typename NodeInfo, typename ArcInfo = EmptyClass,
              typename GraphInfo = EmptyClass>
    class Digraph
        : public BaseGraph<Digraph<NodeInfo, ArcInfo, GraphInfo>,
                           DigraphNode<NodeInfo, ArcInfo, GraphInfo>,
                           DigraphArc<DigraphNode<NodeInfo, ArcInfo, GraphInfo>,
                                      NodeInfo, ArcInfo, GraphInfo>,
                           NodeInfo, ArcInfo>
    {
        using Base =
            BaseGraph<Digraph<NodeInfo, ArcInfo, GraphInfo>,
                      DigraphNode<NodeInfo, ArcInfo, GraphInfo>,
                      DigraphArc<DigraphNode<NodeInfo, ArcInfo, GraphInfo>,
                                 NodeInfo, ArcInfo, GraphInfo>,
                      NodeInfo, ArcInfo>;
    ...
    public:
        Digraph() : info(), num_nodes(0), node_list(), num_arcs(0), arc_list()
        {
            // empty
        }

        Digraph(const GraphInfo& _info) : info(_info), num_nodes(0), num_arcs(0)
        {
            // empty
        }
        ...
        Digraph(const Digraph& g) : info(g.info), num_nodes(0), num_arcs(0)
        {
            copy_graph(g, *this);
        }

        Digraph(Digraph&& g) : Digraph()
        {
            swap(g);
        }
        ...
        Arc* get_first_arc() const
        {
            ...
            return &dl_to_arc(const_cast<DL&>(arc_list).get_next())
                        ->get_item()
                        ->get_item();
        }
