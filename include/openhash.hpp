  ...
  */
  ...
  class OpenAddressingHashTable
      : private DefaultCmpHolder<Cmp>,
        public ContainerAlgorithms<OpenAddressingHashTable<Key, Cmp, Probing>, Key>,
  ...
  private:
    ...
    Key* raw_insert(const Key& k)
    {
      ...
      for (nat_t i = 0; i < cap; ++i)
      {
        ...
        {
          nat_t target = first_tombstone < cap ? first_tombstone : idx;
      ...
      throw std::logic_error("OpenAddressingHashTable: no empty slot found "
  ...
  public:
    ...
    OpenAddressingHashTable(nat_t size, Cmp& _cmp, HashFctType fct,
                            real_t _upper_alpha)
        : table(next_pow2(size), Entry()), num_items(0), num_tombstones(0),
          cmp(_cmp), hash_fct(fct), upper_alpha(_upper_alpha)
    {
      // empty
    }
    ...
    OpenAddressingHashTable(nat_t size, Cmp&& _cmp = Cmp(),
                            HashFctPtr fct = &super_fast_hash)
        : table(next_pow2(size), Entry()), num_items(0), num_tombstones(0),
          cmp(this->default_cmp), hash_fct(fct), upper_alpha(DFT_UPPER_ALPHA)
    {
    ...
    OpenAddressingHashTable(const OpenAddressingHashTable& h)
        : table(h.table), num_items(h.num_items),
          num_tombstones(h.num_tombstones), cmp(cmp_for_copy(*this, h)),
          hash_fct(h.hash_fct), upper_alpha(h.upper_alpha)
    {
      // empty
    }
    ...
    class Iterator : public BidirectionalIterator<Iterator, Key>
    {
      ...
      void skip_backward()
      {
        while (idx > 0 && set_ptr->table[idx - 1].state != OAState::OCCUPIED)
        {
          --idx;
        }
      }
  ...
  template <typename Key, class Cmp = std::equal_to<Key>>
  using QuadraticHashTable = OpenAddressingHashTable<Key, Cmp, QuadraticProbing>;
