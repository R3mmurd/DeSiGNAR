...
namespace Designar
{
  ...
  public:
    ...
    RandTreeNode(Key&& k)
        : BaseNode(std::forward<Key>(k)), count(1)
    {
      // empty
    }

    /** @see RankedTreap's sentinel for why this is fixed at 0. */
    RandTreeNode(BinTreeNodeCtor ctor)
        : BaseNode(ctor), count(0)
    {
      // empty
    }
  ...
  class RandomizedTree : private DefaultCmpHolder<Cmp>,
  ...
  public:
    ...
    RandomizedTree(Cmp& _cmp)
        : RandomizedTree(time(nullptr), _cmp)
    {
      // empty
    }
    ...
    RandomizedTree(RandomizedTree&& t)
        : RandomizedTree()
    {
      swap(t);
    }
  ...
  RandomizedTree<Key, Cmp>::RandomizedTree(const std::initializer_list<Key>& l)
      : RandomizedTree()
  {
  ...
  template <typename Key, class Cmp>
  typename RandomizedTree<Key, Cmp>::Node* RandomizedTree<Key, Cmp>::copy(Node* r)
  {
    if (r == Node::null)
    {
      return Node::null;
