...
namespace Designar
{
  ...
  struct QuadBoundary
  {
    ...
    {
      real_t dx = std::abs(real_t(center.get_x()) - real_t(other.center.get_x()));
      real_t dy = std::abs(real_t(center.get_y()) - real_t(other.center.get_y()));
  ...
  private:
    ...
    void subdivide()
    {
      ...
      northwest = new QuadTree(
          Boundary(PointT(NumberType(cx - h), NumberType(cy + h)), h), depth + 1);
    ...
    explicit QuadTree(const Boundary& _boundary, nat_t _depth)
        : boundary(_boundary), depth(_depth),
  ...
  public:
    explicit QuadTree(const Boundary& _boundary)
        : QuadTree(_boundary, 0)
    {
      // empty
    }

    QuadTree(const QuadTree& t)
        : boundary(t.boundary), points(t.points), depth(t.depth),
          northwest(nullptr), northeast(nullptr),
          southwest(nullptr), southeast(nullptr)
    {
      copy_from(t);
    ...
    bool insert(const PointT& p)
    {
      ...
      if (is_leaf() && (points.size() < NODE_CAPACITY || depth >= MAX_DEPTH))
      {
        points.append(p);
        return true;
      }
