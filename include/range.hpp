...
namespace Designar
{
    ...
    public:
        ...
        Range(T _last) : Range(T(0), _last, T(1))
        {
            // empty
        }

        Range() : Range(std::numeric_limits<T>::max())
        {
            // empty
        }
        ...
        {
            return num_equal(first, r.first) && num_equal(last, r.last) and
                   num_equal(step, r.step);
        }
        ...
        public:
            Iterator(const Range& _r) : r(_r), c(r.min()), p(0)
            {
                // empty
            }

            Iterator(const Range& _r, nat_t pos) : r(_r), c(r.min()), p(0)
            {
                move_to(pos);
            }
