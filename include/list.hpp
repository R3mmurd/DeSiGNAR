...
namespace Designar
{
    ...
    public:
        NodeSLList() : head(nullptr), tail(nullptr)
        {
            // empty
        }
        ...
        NodeSLList(NodeSLList&& l) : NodeSLList()
        {
            swap(l);
        }
    ...
    public:
        ...
        SLList() : BaseList(), num_items(0)
        {
            // empty
        }

        SLList(const SLList& l) : SLList()
        {
            copy_list(l);
        }

        SLList(SLList&& l) : SLList()
        {
            this->swap(l);
        }
        ...
        public:
            Iterator() : list_ptr(nullptr), curr(nullptr), pred(nullptr)
            {
                // empty
            }
    ...
    template <typename T>
    SLList<T>::SLList(const std::initializer_list<T>& l) : SLList()
    {
        for (const T& item : l)
        {
            append(item);
    ...
    public:
        ...
        DLList() : Base(), num_items(0)
        {
            // empty
        }

        DLList(const DLList& l) : DLList()
        {
            copy_list(l);
        }

        DLList(DLList&& l) : DLList()
        {
            swap(l);
        }
        ...
        public:
            Iterator() : Base(), list_ptr(nullptr)
            {
                // empty
            }

            Iterator(const DLList& l)
                : Base(&const_cast<DLList&>(l)),
                  list_ptr(&const_cast<DLList&>(l))
            {
                // empty
            }

            Iterator(const DLList& l, DL* c)
                : Base(&const_cast<DLList&>(l), c),
                  list_ptr(&const_cast<DLList&>(l))
            {
                // empty
            }

            Iterator(const Iterator& it) : Base(it), list_ptr(it.list_ptr)
            {
                // empty
            }
    ...
    template <typename T>
    DLList<T>::DLList(const std::initializer_list<T>& l) : DLList()
    {
        for (const T& item : l)
        {
            append(item);
