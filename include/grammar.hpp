...
namespace Designar
{
  ...
  private:
    ...
    static HashSet<Symbol> first_of_symbol(const Symbol& sym,
  ...
  public:
    ...
    HashSet<Symbol> first_of_sequence(const Sequence& seq, nat_t from,
      ...
      while (changed)
      {
        ...
        for (const Symbol& nt : nonterminals)
        {
          ...
          for (nat_t r = 0; r < rhs_list->size(); ++r)
          {
            ...
            HashSet<Symbol> seq_first = first_of_sequence(rhs, 0, first_sets);
    ...
    HashMap<Symbol, HashSet<Symbol>> compute_follow(
        const HashMap<Symbol, HashSet<Symbol>>& first_sets) const
    {
      ...
      while (changed)
      {
        ...
        {
          const DynArray<Sequence>* rhs_list = productions.search(lhs);
          ...
          for (nat_t r = 0; r < rhs_list->size(); ++r)
          {
            ...
            for (nat_t i = 0; i < rhs.size(); ++i)
            {
              ...
              HashSet<Symbol> beta_first = first_of_sequence(rhs, i + 1, first_sets);
              HashSet<Symbol>& xi_follow = *follow_sets.search(rhs[i]);
              ...
              if (beta_nullable)
              {
                const HashSet<Symbol>& lhs_follow = *follow_sets.search(lhs);
