...
namespace Designar
{
  ...
  {
    using BaseNode = BaseBinTreeNode<Key, AVLNode<Key>,
                                     BinTreeNodeNullValue::SENTINEL>;
  ...
  public:
    ...
    AVLNode(Key&& k)
        : BaseNode(std::forward<Key>(k)), height(0)
    {
      // empty
    }
    ...
    AVLNode(BinTreeNodeCtor ctor)
        : BaseNode(ctor), height(-1)
    {
      // empty
    }
  ...
  AVLTree<Key, Cmp>::AVLTree(const std::initializer_list<Key>& l)
      : AVLTree()
  {
