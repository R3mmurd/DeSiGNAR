...
namespace Designar
{
  ...
  public:
    ...
    bool remove(const Key& k)
    {
      ...
      {
        // Everything in L(old_root) is < k, so splaying it with k drives
        // it as far right as possible — i.e. brings L(old_root)'s
        // maximum to its top — which is exactly the node that should
        // become the new root once old_root is spliced out.
