...
namespace Designar
{
  ...
  public:
    ...
    TreapNode(Key&& k)
        : BaseNode(std::forward<Key>(k)), prior(0)
    {
      // empty
    }

    TreapNode(BinTreeNodeCtor ctor)
        : BaseNode(ctor), prior(rng_t::max())
    {
      // empty
    }
  ...
  public:
    ...
    Treap(const Treap& t)
        : head(), root(L(&head)), cmp(cmp_for_copy(*this, t)),
          num_items(t.num_items), rng(time(nullptr))
    {
      root = copy(t.root);
    }
  ...
  Treap<Key, Cmp>::Treap(const std::initializer_list<Key>& l)
      : Treap()
  {
  ...
  Treap<Key, Cmp>::exclusive_join(Node*& ts, Node*& tg)
  {
    if (ts == Node::null)
    {
      return tg;
  ...
  template <typename Key, class Cmp>
  typename Treap<Key, Cmp>::Node*
  Treap<Key, Cmp>::insert(Node*& r, Node* p, Cmp& cmp)
  {
    if (r == Node::null)
    {
      r = p;
