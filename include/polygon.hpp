...
namespace Designar
{
    ...
    public:
        ...
        GenPolygon() : num_vertices(0), vertex_list()
        {
            // empty
        }
        ...
        GenPolygon(const GenPolygon& p) : GenPolygon()
        {
            copy(p);
        }

        GenPolygon(GenPolygon&& p) : GenPolygon()
        {
            swap(p);
        }
        ...
        const PointT& get_first_vertex() const
        {
            ...
            return dl_to_vertex(const_cast<DL*>(vertex_list.get_next()))
                ->get_item();
        }
        ...
        const PointT& get_last_vertex() const
        {
            ...
            return dl_to_vertex(const_cast<DL*>(vertex_list.get_prev()))
                ->get_item();
        }
        ...
        SegmentType get_first_segment() const
        {
            ...
            const PointT& s =
                dl_to_vertex(const_cast<DL*>(vertex_list.get_next()))
                    ->get_item();
            const PointT& t =
                dl_to_vertex(
                    const_cast<DL*>(vertex_list.get_next()->get_next()))
                    ->get_item();
        ...
        SegmentType get_last_segment() const
        {
            ...
            const PointT& s =
                dl_to_vertex(
                    const_cast<DL*>(vertex_list.get_prev())->get_prev())
                    ->get_item();
            const PointT& t =
                dl_to_vertex(const_cast<DL*>(vertex_list.get_prev()))
                    ->get_item();
        ...
        public:
            VertexIterator() : Base(), p_ptr(nullptr)
            {
                // empty
            }
            ...
            VertexIterator(const VertexIterator& it) : Base(it), p_ptr(it.p_ptr)
            {
                // empty
            }

            VertexIterator(VertexIterator&& it) : VertexIterator()
            {
                swap(it);
            }
        ...
        public:
            SegmentIterator() : head(nullptr), curr(nullptr)
            {
                // empty
            }
            ...
            SegmentIterator(SegmentIterator&& it) : SegmentIterator()
            {
                swap(it);
            }
            ...
            SegmentType get_current()
            {
                ...
                Vertex* next = curr->get_next() == head ? head->get_next()
                                                        : curr->get_next();
        ...
        {
            return all_it(vertices_begin(), vertices_end(),
        ...
        {
            return all_it(segments_begin(), segments_end(),
                          std::forward<Pred>(pred));
        }
    ...
    bool point_in_polygon(const GenPolygon<PointT>& poly, const PointT& p)
    {
        ...
        poly.for_each_segment(
            [&](const GenSegment<PointT>& seg)
            {
                const PointT& a = seg.get_src_point();
                const PointT& b = seg.get_tgt_point();

                bool straddles =
                    (a.get_y() > p.get_y()) != (b.get_y() > p.get_y());
                ...
                real_t x_intersect =
                    real_t(a.get_x()) + real_t(p.get_y() - a.get_y()) *
