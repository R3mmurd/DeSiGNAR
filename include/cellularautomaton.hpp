...
namespace Designar
{
    ...
    public:
              ...
              strides(_extents.size(), nat_t(0))
        {
            ...
            {
                throw std::domain_error(
                    "Lattice: must have at least one dimension");
            }
            ...
            for (nat_t i = 0; i < num_dims; ++i)
            {
                ...
                {
                    throw std::domain_error(
                        "Lattice: every extent must be positive");
                }
    ...
    public:
        ...
        virtual DynArray<CellT>
        get_neighbors(const Lattice<CellT>& lattice,
    ...
    public:
        CellularAutomaton(const Lattice<CellT>& _lattice,
                          ...
                          const Neighborhood<CellT>& _neighborhood,
                          RuleFn _rule)
            : lattice(_lattice),
              boundary(_boundary),
        ...
        void step()
        {
            ...
            for (nat_t idx = 0; idx < lattice.size(); ++idx)
            {
                ...
                DynArray<CellT> neighbors =
                    neighborhood.get_neighbors(lattice, boundary, pos);
