...
namespace Designar
{
    ...
    public:
        LHashTable(nat_t size, Cmp& _cmp, HashFctType fct, real_t _lower_alpha,
                   real_t _upper_alpha)
            : BaseArray(size),
              num_items(0),
              cmp(_cmp),
        ...
        LHashTable(Cmp&& _cmp, HashFctType fct)
            : LHashTable(DFT_SIZE, std::forward<Cmp>(_cmp), fct,
        ...
        LHashTable(LHashTable&& h) : LHashTable()
        {
            swap(h);
        }
        ...
        protected:
            ...
            Iterator(const LHashTable& h, int)
                : set_ptr(&const_cast<LHashTable&>(h)),
                  set_pos(h.M()),
                  list_it(),
                  pos(h.N())
            {
