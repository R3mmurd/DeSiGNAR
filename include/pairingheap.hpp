...
namespace Designar
{
  ...
  public:
    class Node
    {
      ...
      Node(const Key& k)
          : key(k), child(nullptr), sibling(nullptr)
      {
        // empty
      }
  ...
  public:
    PairingHeap(Cmp& _cmp)
        : root(nullptr), num_items(0), cmp(_cmp)
    {
      // empty
    }
    ...
    PairingHeap(const PairingHeap& h)
        : root(copy(h.root)), num_items(h.num_items), cmp(cmp_for_copy(*this, h))
    {
      // empty
    }
  ...
  private:
    ...
    static bool unlink_from_parent(Node* subtree, Node* target)
    {
      ...
      for (Node* c = subtree->child; c != nullptr; prev = c, c = c->sibling)
      {
        if (c == target)
        {
          if (prev == nullptr)
