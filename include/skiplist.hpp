...
namespace Designar
{
                   ...
                   public SetAlgorithms<SkipList<Key, Cmp>, Key>
  {
    ...
    struct Node
    {
      ...
      Node(const Key& k, nat_t lvl)
          : key(k), forward(lvl, nullptr)
      {
        // empty
      }

      Node(nat_t lvl)
          : key(), forward(lvl, nullptr)
      {
        // empty
      }
    };
    ...
    DynArray<Node*> locate_update(const Key& k) const
    {
      ...
      {
        while (curr->forward[i] != nullptr && cmp(curr->forward[i]->key, k))
        {
          curr = curr->forward[i];
        }
  ...
  public:
    ...
    SkipList(Cmp& _cmp)
        : header(new Node(MAX_LEVEL)), level(1), num_items(0), cmp(_cmp),
          rng(time(nullptr))
    {
      // empty
    }
    ...
    SkipList(Cmp&& _cmp = Cmp())
        : header(new Node(MAX_LEVEL)), level(1), num_items(0),
          cmp(this->default_cmp), rng(time(nullptr))
    {
    ...
    SkipList(const SkipList& t)
        : header(new Node(MAX_LEVEL)), level(1), num_items(0),
          cmp(cmp_for_copy(*this, t)), rng(time(nullptr))
    {
      for (const Key& k : t)
      {
        insert(k);
