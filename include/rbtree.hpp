...
namespace Designar
{
  ...
  {
    using BaseNode = BaseBinTreeNode<Key, RbNode<Key>,
                                     BinTreeNodeNullValue::SENTINEL>;
  ...
  public:
    ...
    RbNode(const Key& k)
        : BaseNode(k), color(RBColor::RED)
    {
      // empty
    }

    RbNode(Key&& k)
        : BaseNode(std::forward<Key>(k)), color(RBColor::RED)
    {
      // empty
    }
    ...
    RbNode(BinTreeNodeCtor ctor)
        : BaseNode(ctor), color(RBColor::BLACK)
    {
      // empty
    }
  ...
  public:
    ...
    RbTree(Cmp& _cmp)
        : head(), root(L(&head)), cmp(_cmp), num_items(0)
    {
      // empty
    }
  ...
  RbTree<Key, Cmp>::RbTree(const std::initializer_list<Key>& l)
      : RbTree()
  {
  ...
  RbTree<Key, Cmp>::insert(Node*& h, Node* p, Cmp& cmp)
  {
    if (h == Node::null)
    {
      h = p;
