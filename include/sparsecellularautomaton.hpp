...
namespace Designar
{
    ...
    void validate_coordinates(const SparseCoordinates& coords) const
    {
      if (coords.size() != num_dims)
      {
        throw std::domain_error(
            "SparseLattice: coordinate count does not match dimensionality");
      }
    }
  ...
  public:
    explicit SparseLattice(nat_t _num_dims, const CellT& _background = CellT())
  ...
  public:
    ...
    virtual DynArray<SparseCoordinates> get_neighbor_coordinates(
        nat_t num_dims, const SparseCoordinates& pos) const = 0;
  };
  ...
  {
  public:
    DynArray<SparseCoordinates> get_neighbor_coordinates(
        nat_t num_dims, const SparseCoordinates& pos) const override
    {
  ...
  public:
    SparseCellularAutomaton(const SparseLattice<CellT>& _lattice,
                            ...
                            RuleFn _rule)
        : lattice(_lattice), neighborhood(_neighborhood), rule(std::move(_rule)),
          generation(0)
    {
      // empty
    }
      ...
      lattice.for_each_active(
          [&](const typename SparseLattice<CellT>::Entry& e)
          {
            ...
            for (const SparseCoordinates& n :
                 neighborhood.get_neighbor_coordinates(num_dims, e.coords))
            {
