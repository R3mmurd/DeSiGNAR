...
namespace Designar
{
  ...
  public:
    DFA(nat_t _num_states, StateId _start)
        : num_states(_num_states), start_state(_start),
  ...
  public:
    ...
    HashSet<StateId> epsilon_closure(const HashSet<StateId>& states) const
    {
      ...
      while (!pending.is_empty())
      {
        ...
        const DynArray<StateId>* targets = transitions[s].search(EPSILON);
    ...
    DynArray<StateId> move(const HashSet<StateId>& states, const Symbol& sym) const
    {
  ...
  class RegexToNFA
  {
    ...
    Fragment parse_base()
    {
      if (peek() == '(')
      {
        ...
        if (peek() != ')')
        {
          throw std::domain_error("regex_to_nfa: unbalanced parentheses");
        }
      ...
      if (peek() == '\0')
      {
        throw std::domain_error("regex_to_nfa: unexpected end of pattern");
      }
      ...
      if (c == '\\')
      {
        if (peek() == '\0')
        {
          throw std::domain_error("regex_to_nfa: dangling escape at end of pattern");
        }
  ...
  public:
    ...
    NFA<char> build()
    {
      ...
      if (pos != pattern.size())
      {
        throw std::domain_error("regex_to_nfa: unexpected character in pattern");
      }
