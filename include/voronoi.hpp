...
namespace Designar
{
  ...
  class VoronoiDiagram
  {
    ...
    static PointT intersect_bisector(const PointT& a, const PointT& b,
                                     const PointT& site, const PointT& other)
    {
      ...
      real_t x = real_t(a.get_x()) + t * (real_t(b.get_x()) - real_t(a.get_x()));
      real_t y = real_t(a.get_y()) + t * (real_t(b.get_y()) - real_t(a.get_y()));
  ...
  public:
        ...
        : sites(_sites), bounds_half_size(_bounds_half_size)
    {
      if (sites.size() < 2)
      {
        throw std::domain_error("VoronoiDiagram needs at least two sites");
      }
    }
