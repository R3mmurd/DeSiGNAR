...
namespace Designar
{
    ...
    public:
        ...
        GenPoint2D(const NumT& _x, const NumT& _y) : x(_x), y(_y)
        {
            // empty
        }

        GenPoint2D(const NumT& _x, NumT&& _y) : x(_x), y(std::forward<NumT>(_y))
        {
            // empty
        }

        GenPoint2D(NumT&& _x, const NumT& _y) : x(std::forward<NumT>(_x)), y(_y)
        {
            // empty
        }
        ...
        GenPoint2D(const GenPoint2D& p) : x(p.x), y(p.y)
        {
            // empty
        }

        GenPoint2D(GenPoint2D&& p) : GenPoint2D()
        {
            swap(p);
        }
