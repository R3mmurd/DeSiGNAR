...
namespace Designar
{
  ...
  private:
    ...
    Symbol read_tape(const DynArray<Symbol>& left, const DynArray<Symbol>& right,
                     int_t pos) const
    {
      if (pos >= 0)
      {
    ...
    void write_tape(DynArray<Symbol>& left, DynArray<Symbol>& right, int_t pos,
  ...
  public:
    TuringMachine(nat_t _num_states, StateId _start, const Symbol& _blank)
        : num_states(_num_states), start_state(_start), blank(_blank),
          transitions(_num_states, HashMap<Symbol, Transition>())
    {
      if (_start >= _num_states)
      {
        throw std::domain_error("TuringMachine: start state out of range");
      }
    }
