...
namespace Designar
{
  ...
  private:
    static Result brute_force(const DynArray<PointT>& pts, nat_t lo, nat_t hi)
    {
      Result best{pts[lo], pts[lo + 1], pts[lo].distance_with(pts[lo + 1])};
    ...
    static Result solve(const DynArray<PointT>& by_x, nat_t lo, nat_t hi)
    {
      ...
      for (nat_t i = lo; i <= hi; ++i)
      {
        if (std::abs(real_t(by_x[i].get_x()) - real_t(mid_x)) < best.distance)
        {
          strip.append(by_x[i]);
        }
      }
      ...
      for (nat_t i = 0; i < strip.size(); ++i)
      {
        for (nat_t j = i + 1;
             j < strip.size() &&
             real_t(strip[j].get_y() - strip[i].get_y()) < best.distance;
             ++j)
        {
  ...
  public:
    static Result compute(DynArray<PointT> points)
    {
      if (points.size() < 2)
      {
        throw std::domain_error("ClosestPair needs at least two points");
      }
