  ...
  class BipartiteMatching
  {
    ...
    bool try_augment(Node<GT>* u, HashSet<Node<GT>*>& visited)
    {
      for (AdArcIt<GT> it(g, u); it.has_current(); it.next())
      {
        ...
        if (matched_to_v == nullptr || try_augment(*matched_to_v, visited))
        {
          // operator[] (insert-or-update), not insert(): v (and u, if it
          // was previously matched to someone else before this
          // augmenting path stole it away) may already be a key in
          // `match` from an earlier augmentation — insert() silently
          // does nothing when the key already exists, which left stale
          // pairings in place instead of rewriting them.
          match[v] = u;
          match[u] = v;
          return true;
        }
