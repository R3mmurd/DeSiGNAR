...
namespace Designar
{
    ...
    public:
        ...
        static nat_t optimal_num_bits(nat_t expected_items,
                                      real_t false_positive_rate)
        {
            if (expected_items == 0)
            {
            ...
            real_t m =
                -(real_t(expected_items) * std::log(false_positive_rate)) /
                (std::log(2.) * std::log(2.));
        ...
        static nat_t optimal_num_hashes(nat_t num_bits, nat_t expected_items)
        {
            ...
            real_t k =
                (real_t(num_bits) / real_t(expected_items)) * std::log(2.);
