...
namespace Designar
{
  /** The minimax value of `state` (from the maximizing player's point
      ...
      searched. */
  template <typename State, class GetChildren, class IsTerminal, class Evaluate>
  real_t alpha_beta_search(const State& state, int_t depth, real_t alpha, real_t beta,
                           bool maximizing_player, GetChildren&& get_children,
                           IsTerminal&& is_terminal, Evaluate&& evaluate)
  {
    ...
    if (maximizing_player)
    {
      ...
      {
        real_t child_value =
            alpha_beta_search(children[i], depth - 1, alpha, beta, false,
    ...
    {
      real_t child_value =
          alpha_beta_search(children[i], depth - 1, alpha, beta, true, get_children,
  ...
  template <typename State, class GetChildren, class IsTerminal, class Evaluate>
  MoveSearchResult<State> find_best_move(const State& state, int_t depth,
                                         bool maximizing_player, GetChildren&& get_children,
                                         IsTerminal&& is_terminal, Evaluate&& evaluate)
  {
    DynArray<State> children = get_children(state);
    ...
    real_t best_score = maximizing_player ? -std::numeric_limits<real_t>::infinity()
    ...
    {
      real_t score =
          alpha_beta_search(children[i], depth - 1, alpha, beta, !maximizing_player,
                                   get_children, is_terminal, evaluate);

      bool better = maximizing_player ? (score > best_score) : (score < best_score);
