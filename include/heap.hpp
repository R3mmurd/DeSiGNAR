...
namespace Designar
{
    ...
    public:
        ...
        FixedHeap(Cmp& _cmp) : num_items(0), cmp(_cmp)
        {
            // empty
        }
        ...
        /** See GenArraySet's matching constructor for why this cannot simply
            ...
            instead of a move. */
        FixedHeap(Cmp&& _cmp = Cmp()) : num_items(0), cmp(this->default_cmp)
        {
        ...
        FixedHeap(FixedHeap&& h) : FixedHeap()
        {
            swap(h);
        }
    ...
    public:
        ...
        DynHeap(Cmp& _cmp) : BaseArray(), cmp(_cmp)
        {
            // empty
        }
        ...
        /** See GenArraySet's matching constructor for why this cannot simply
            ...
            instead of a move. */
        DynHeap(Cmp&& _cmp = Cmp()) : BaseArray(), cmp(this->default_cmp)
        {
            this->default_cmp = _cmp;
        }

        DynHeap(const DynHeap& h) : BaseArray(h), cmp(cmp_for_copy(*this, h))
        {
            // empty
        }

        DynHeap(DynHeap&& h) : DynHeap()
        {
            swap(h);
        }
    ...
    class HeapNode : public BaseBinTreeNode<Key, HeapNode<Key>,
                                            BinTreeNodeNullValue::NULLPTR>
    {
        using BaseNode =
        ...
        struct HeapNodeBits
        {
            ...
            HeapNodeBits() : is_left(true), is_leaf(true)
            {
                // empty
            }
        };
    ...
    public:
        ...
        HeapNode(const Key& k) : BaseNode(k)
        {
            // empty
        }

        HeapNode(Key&& k) : BaseNode(std::forward<Key>(k))
        {
            // empty
        }

        HeapNode(BinTreeNodeCtor ctor) : BaseNode(ctor)
        {
            // empty
        }
