...
int main()
{
  ...
  // works (the set observes changes to the caller's comparator object).
  {
    ...
    ReverseAfterN cmp(0); // always "a > b" (count starts at 0, immediately > flip_at)
    ...
    assert(cmp.count > 0); // the set actually used *our* external cmp object
  }
