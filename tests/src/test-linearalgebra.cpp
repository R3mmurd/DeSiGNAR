...
int main()
{
    // Vector arithmetic.
    {
        ...
        assert(close(diff[0], 3.0) && close(diff[1], 3.0) &&
               close(diff[2], 3.0));

        Vector<real_t> scaled = 2.0 * u;
        assert(close(scaled[0], 2.0) && close(scaled[1], 4.0) &&
               close(scaled[2], 6.0));
    ...
    // solve: a hand-verifiable 2x2 system.
    {
        ...
        cout << "solve: hand-verified 2x2 system + singular detection "
                "Everything ok!\n";
    }
    ...
    // inverse: A * inverse(A) == I, on a hand-verifiable 2x2 example.
    {
        ...
        cout << "inverse: hand-verified 2x2 example, A*inverse(A)==I "
                "Everything ok!\n";
    }
