...
int main()
{
    ...
    cout << "observed false-positive rate: " << observed_fp << " (target "
         << target_fp << ")\n";
    assert(observed_fp <
           target_fp *
               5); // generous margin, still catches a badly broken filter

    bf.clear();
    assert(bf.size() == 0);
    for (int_t x : inserted)
        assert(!bf.might_contain(x)); // cleared: no false claims of membership
                                      // (probabilistically expected all-empty)
