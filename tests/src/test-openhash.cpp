...
int main()
{
  ...
  HashSet<int_t, std::equal_to<int_t>, LinearHashTable> hs_linear = {1, 2, 3, 4};
  assert(hs_linear.equal({1, 2, 3, 4}));

  HashSet<int_t, std::equal_to<int_t>, QuadraticHashTable> hs_quad = {1, 2, 3, 4};
  assert(hs_quad.equal({1, 2, 3, 4}));

  HashSet<int_t, std::equal_to<int_t>, DoubleHashingTable> hs_double = {1, 2, 3, 4};
