    ...
    for (int_t t = 0; t < 8; ++t)
      threads.emplace_back([&]
        ...
        for (int_t i = 0; i < 1000; ++i)
          if (shared_set.insert(i)) // many threads race to insert the SAME 1000 keys
            success_count.fetch_add(1); });
    for (auto& th : threads)
      th.join();

    assert(shared_set.size() == 1000);
    assert(success_count.load() == 1000); // exactly one thread wins each key
  }
