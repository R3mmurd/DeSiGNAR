...
int main()
{
  ...
  for (const string& s : {string("0"), string("1"), string("10"),
                          string("0110"), string("001"), string("0111"),
                          string("00011"), string("000111000")})
  {
    auto result = tm.run(s);
    assert(!result.accepted);
  }
  ...
  // is exact.
  {
    ...
    cout << "TuringMachine: trivial writer, exact tape/step-count Everything ok!\n";
  }
  ...
  // reject immediately (0 steps), not loop or crash.
  {
    ...
    tm2.set_accepting(0); // never reached, since it rejects immediately below instead
    ...
    cout << "TuringMachine: immediate reject (no matching transition) Everything ok!\n";
  }
  ...
  // hanging the test suite forever.
  {
    ...
    cout << "TuringMachine: non-halting machine caught by max_steps Everything ok!\n";
  }
