...
int main()
{
  ...
  // counterparts on the same graph.
  {
    ...
    depth_first_traverse_prefix_iterative(g, [&](Node<GT>* p)
                                          { prefix_it.append(p->get_info()); });
    depth_first_traverse_suffix(g, [&](Node<GT>* p)
                                { suffix_rec.append(p->get_info()); });
    depth_first_traverse_suffix_iterative(g, [&](Node<GT>* p)
                                          { suffix_it.append(p->get_info()); });
  ...
  // the call stack, must still be fully traversable.
  {
    ...
    nat_t count = 0;
    depth_first_traverse_prefix_iterative(chain, [&](Node<GT>*)
                                          { ++count; });
  ...
  for (bool c_edge_first : {true, false})
  {
    ...
    if (c_edge_first)
      g.insert_arc(b, c, 0); // C is a leaf hanging off B: B must be a cut node
    ...
    bool b_is_cut = cut_nodes.exists([&](Node<GT>* n)
                                     { return n == b; });
    assert(b_is_cut);
  }
