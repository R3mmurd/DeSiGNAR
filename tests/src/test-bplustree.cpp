...
int main()
{
    ...
    // Unlike BTree's for_each_inorder (which recurses through every
    // internal node), this walks the flat leaf chain — the count/order
    // checks here are also an indirect check that the chain wasn't
    // broken by any of the splits above.
    int_t prev = -1;
    nat_t count = 0;
    t.for_each_inorder(
        [&](int_t x)
    ...
    for (int_t k : keys)
    {
        ...
        assert(t.verify()); // includes verify_leaf_chain()
    }
    assert(t.is_empty());
    assert(!t.remove(0));

    // interleaved insert/remove stress with different min-degree values —
    // the leaf chain (relinked on every split/merge) is exactly the part
    // BTree's own equivalent stress test doesn't need to exercise.
    {
        BPlusTree<int_t, std::less<int_t>, 2>
    ...
    BPlusTree<int_t> t5 = std::move(t4);
    assert(t5.search(99999) != nullptr);
    assert(t5.verify());

    cout << "BPlusTree: Everything ok!\n";
    return 0;
}
