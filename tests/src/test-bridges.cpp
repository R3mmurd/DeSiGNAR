...
int main()
{
  ...
  // Bridges: A-B, B-C, C-F. Non-bridges: B-D, D-E, E-B (all part of the cycle).
  GT g;
  ...
  {
    return bridges.exists([&](Arc<GT>* a2)
                          { return a2 == target; });
  };
  ...
  assert(matched_pairs == 3); // this instance has a perfect matching on the left

  cout << "BipartiteMatching: Everything ok! (matched " << matched_pairs << " pairs)\n";
  return 0;
}
