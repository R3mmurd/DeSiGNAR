...
int main()
{
  ...
  assert(set_equals(*first.search("E'"), DynArray<string>({"+", Grammar::EPSILON})));
  assert(set_equals(*first.search("T'"), DynArray<string>({"*", Grammar::EPSILON})));

  cout << "Grammar: FIRST sets (Dragon Book expression grammar) Everything ok!\n";
  ...
  assert(set_equals(*follow.search("T'"), DynArray<string>({"+", ")", "$"})));
  assert(set_equals(*follow.search("F"), DynArray<string>({"+", "*", ")", "$"})));

  cout << "Grammar: FOLLOW sets (Dragon Book expression grammar) Everything ok!\n";
  ...
  // and a sequence starting with a non-nullable symbol stops there.
  {
    ...
    HashSet<string> f2 = g.first_of_sequence(starts_with_terminal, 0, first);
  ...
  //   S -> c
  {
    ...
    cout << "Grammar: FIRST/FOLLOW without epsilon productions Everything ok!\n";
  }
