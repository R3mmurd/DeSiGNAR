...
int main()
{
    ...
    DynArray<Point2D> pts = {Point2D(0., 0.), Point2D(10., 10.),
                             Point2D(0.1, 0.1), Point2D(5., 5.)};
    auto result = ClosestPair<Point2D>::compute(pts);
    assert(num_equal(result.distance,
                     Point2D(0., 0.).distance_with(Point2D(0.1, 0.1))));
    ...
    for (int_t trial = 0; trial < 100; ++trial)
    {
        ...
        for (nat_t i = 0; i < random_pts.size(); ++i)
            for (nat_t j = i + 1; j < random_pts.size(); ++j)
                brute_best = std::min(
                    brute_best, random_pts[i].distance_with(random_pts[j]));
