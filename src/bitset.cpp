...
namespace Designar
{
    ...
    Byte::Byte(bool _b1, bool _b2, bool _b3, bool _b4, bool _b5, bool _b6,
               bool _b7, bool _b8)
        : byte(static_cast<unsigned char>(
              (_b1 ? 1 : 0) << 0 | (_b2 ? 1 : 0) << 1 | (_b3 ? 1 : 0) << 2 |
              (_b4 ? 1 : 0) << 3 | (_b5 ? 1 : 0) << 4 | (_b6 ? 1 : 0) << 5 |
              (_b7 ? 1 : 0) << 6 | (_b8 ? 1 : 0) << 7))
    {
        // Empty
    }
    ...
    Byte::Byte(const Byte& b) : byte(b.byte)
    {
        // Empty
    }
    ...
    DynBitSet::RWProxy::RWProxy(DynBitSet& _dbs, nat_t _i) : dbs(_dbs), i(_i)
    {
        // empty
    }
    ...
    DynBitSet::DynBitSet() : num_bits(0), bit_array()
    {
        // empty
    }

    DynBitSet::DynBitSet(nat_t nb, bool val) : DynBitSet()
    {
        init(nb, val);
    }
    ...
    DynBitSet::DynBitSet(DynBitSet&& dbs) : DynBitSet()
    {
        swap(dbs);
    }
    ...
    void DynBitSet::append(bool value)
    {
        ...
        Byte& byte = byte_num < bit_array.size() ? bit_array[byte_num]
        ...
        byte.set_bit(static_cast<unsigned char>(which_bit_in_byte(num_bits)),
                     value);
        ++num_bits;
    }
    ...
    bool DynBitSet::remove_last()
    {
        ...
        bool ret_val = byte.get_bit(
            static_cast<unsigned char>(which_bit_in_byte(num_bits - 1)));
        --num_bits;
        return ret_val;
    }
